# Canvas LMS API Reference - Universidad Autónoma de Chile

This document contains the complete Canvas API endpoint documentation for the Early Warning System project. It was extracted from the project's `CLAUDE.md` to keep operational instructions lean and separate from reference material.
...
5. **Page Views API** has no course_id filter -- must parse URLs post-fetch
6. **"Libro de Calificaciones"** is an external LTI tool -- grades there are NOT accessible via Canvas API
7. **Different courses use different grade scales** (1-7 Chilean vs 0-100%)
8. **25/31 Control de Gestion courses** lacked sufficient grade data
9. **Best prediction results** come from courses with 20-80% pass rates

---

*Extracted from CLAUDE.md on 2026-02-11 for cleaner project memory structure.*
